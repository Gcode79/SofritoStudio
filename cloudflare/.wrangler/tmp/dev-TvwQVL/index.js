...
// .wrangler/tmp/bundle-5kYkxR/strip-cf-connecting-ip-header.js
...
async function maybeInjectRecipeCta(request, env, path) {
  ...
  if (!unlock) {
    console.log("CTA-debug no-unlock", path, slug, Object.keys(RECIPE_UNLOCKS).length);
    return null;
  }
  const response = await env.ASSETS.fetch(request);
  const contentType = response.headers.get("Content-Type") || "";
...
// .wrangler/tmp/bundle-5kYkxR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5kYkxR/middleware-loader.entry.ts
